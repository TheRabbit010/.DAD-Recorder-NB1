...
# ==========================================
# 3. โครงสร้าง Mapping สัญญาณ (💡 อัปเดตใหม่!)
# ขยับ +1 เพื่อดึงค่า "MAX" ของแต่ละ Channel แทนค่า MIN (ที่เป็น 0)
# ==========================================
col_mapping = {
...
if uploaded_files:
    ...
    for file, start_dt, is_valid in parsed_files:
        try:
            ...
            # 💡 คืนค่า order='C' (อ่านบรรทัดต่อบรรทัดตามเวลาจริง)
            ...
            for ch_num, col_idx in col_mapping.items():
                if col_idx < TOTAL_CHANNELS:
                    ...
                    # กรองเฉพาะค่า Error ขยะที่เกินจริงออก (กลับไปใช้สูตรเดิมของคุณ)
    ...
    if all_dfs:
        ...
        # จัดเรียงเวลาใหม่ ป้องกันเส้นกราฟตีกลับหลัง
        ...
        st.sidebar.download_button(label="📥 ดาวน์โหลดข้อมูล (.csv)", data=csv_data, file_name=f"DAD_Export_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv", mime="text/csv", use_container_width=True)
        ...
        def apply_white_theme_style(fig, y_title, y_range=None, is_secondary=False):
            fig.update_layout(height=380, template="plotly_white", margin=dict(l=40, r=40, t=30, b=30), hovermode="x unified", legend=dict(x=1.01, y=1, xanchor="left", yanchor="top", font=dict(size=11)))
        ...
        for idx, col in enumerate(top_names):
            if col in full_df.columns:
                fig_top.add_trace(go.Scatter(x=full_df["Datetime"], y=full_df[col], name=col, mode="lines", line=dict(width=1.8, color=COLOR_PALETTE[idx]), hovertemplate=f"{col}: %{{y:.1f}} °C<extra></extra>"))
        ...
        for idx, col in enumerate(bot_names):
            if col in full_df.columns:
                fig_bot.add_trace(go.Scatter(x=full_df["Datetime"], y=full_df[col], name=col, mode="lines", line=dict(width=1.8, color=COLOR_PALETTE[idx]), hovertemplate=f"{col}: %{{y:.1f}} °C<extra></extra>"))
        ...
        if "Dryer #1" in full_df.columns: fig_dryer.add_trace(go.Scatter(x=full_df["Datetime"], y=full_df["Dryer #1"], name="Dryer #1", mode="lines", line=dict(color="#2ecc71", width=2.0)))
        if "Dryer #2" in full_df.columns: fig_dryer.add_trace(go.Scatter(x=full_df["Datetime"], y=full_df["Dryer #2"], name="Dryer #2", mode="lines", line=dict(color="#e67e22", width=2.0)))
        ...
        if "O2 Exit" in full_df.columns: fig_o2_n2.add_trace(go.Scatter(x=full_df["Datetime"], y=full_df["O2 Exit"], name="O2 Exit", mode="lines", line=dict(color="#e74c3c", width=2.0)), secondary_y=False)
        if "O2 Entrance" in full_df.columns: fig_o2_n2.add_trace(go.Scatter(x=full_df["Datetime"], y=full_df["O2 Entrance"], name="O2 Entrance", mode="lines", line=dict(color="#3498db", width=2.0)), secondary_y=False)
        if "N2 Flow" in full_df.columns: fig_o2_n2.add_trace(go.Scatter(x=full_df["Datetime"], y=full_df["N2 Flow"], name="N2 Flow", mode="lines", line=dict(color="#2ecc71", width=1.5, dash="dash")), secondary_y=True)
        ...
        if "Dew Point" in full_df.columns: fig_dew.add_trace(go.Scatter(x=full_df["Datetime"], y=full_df["Dew Point"], name="Dew Point", mode="lines", line=dict(color="#9b59b6", width=2.0)))
