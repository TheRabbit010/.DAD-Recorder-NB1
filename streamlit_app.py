...
st.subheader("โหมดอัตโนมัติ 100%: แสดงแกน Date & Time แยกอิสระครบทุกกล่องย่อย")
...
if uploaded_file is not None:
    ...
    if len(clean_stream) >= detected_channels:
        ...
        st.success(f"🔓 แตกบล็อกพารามิเตอร์และสอบเทียบสเกลสำเร็จ! แยกขอบเขตตัวเลขและแกนเวลาทุกแถวเรียบร้อย")

        # 2. เริ่มสร้างโครงสร้าง Subplots แบบ 5 ชั้นแนวตั้ง 
        # [แก้ไขจุดสำคัญ] ปรับ shared_xaxes=False เพื่อยกเลิกการยุบแกนเวลามารวมกันด้านล่างสุด
        fig = make_subplots(
            rows=5, cols=1, 
            shared_xaxes=False, 
            vertical_spacing=0.08, # เพิ่มระยะห่างระหว่างกล่องเล็กน้อยเพื่อให้ตัวเลขเวลาไม่เบียดกัน
        ...
        # กล่องที่ 4: Oxygen Entrance & Exit และ N2 Flow
        ...
        # ปรับล็อกช่วงกรอบสเกลแกน Y ให้ตรงตามมาตรฐานขอบเขตจริงของเครื่องจักร
        ...
        # ----------------------------------------------------
        # [จุดแก้ไขเด็ดขาด] สั่งเปิดข้อความตัวเลข Date & Time กำกับไว้ที่ด้านล่างของทุกกล่องย่อย
        # ----------------------------------------------------
        for r in range(1, 6):
