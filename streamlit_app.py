...
st.title("🏭 Factory Process Master Dashboard - Calibrated Real Scale")
st.subheader("ระบบถอดรหัสพิกัด Yokogawa .DAD ล็อกสเกลจริงและจัดกลุ่มคำอธิบายไว้ขวาสุด")
...
if uploaded_file is not None:
    ...
    # 1. ลอจิกขูดสตรีมตัวเลขความละเอียดสูงจากไฟล์ดิบ (กวาดเอาตัวเลขทั้งหมดมาหั่นตารางเป็นบล็อก)
    ...
    if len(clean_stream) >= detected_channels:
        ...
        # ⏱️ แถบตั้งค่ากะเวลาทำงานซิงค์ออโต้ตามที่ปรากฏในเครื่องบันทึกจริง (Aug 12, 2026 เริ่มกะเวลา 01:30:00)
        ...
        # ----------------------------------------------------
        # [จุดแก้ไขสำคัญที่สุด] Industrial Scale Multiplier Engine & Target Mapping 
        # ทำการซิงค์ผังช่องสัญญาณเข้าดัชนีคอลัมน์ และคูณปรับฐานสเกลกลับสู่ตัวเลขจริงหน้างาน
        # ----------------------------------------------------
        ...
        st.success(f"🔓 แตกบล็อกพารามิเตอร์และแมปปิ้งสเกลอุตสาหกรรมเสร็จสมบูรณ์! ({len(df)} แถวข้อมูล)")
        ...
        # กล่องที่ 4: Oxygen Entrance & Exit [แกนซ้าย ล็อกสเกลช่วง 0-200 ppm] และ N2 Flow [แกนขวาออโต้สเกลแยกอิสระ]
        ...
        # กล่องที่ 5: Dew Point -> โหมด Free Scale ขยับตามคลื่นดิบจริง
        ...
        fig.update_layout(
            ...
            title_text="Yokogawa Process Analyzer Dashboard (Precision Industrial Calibrated)",
            ...
            legend4=dict(traceorder="normal", x=1.02, y=0.35, bgcolor="rgba(0,0,0,0)"), # รวม Oxygen & N2 Flow ไว้บล็อกขวาของชั้นตัวเอง
            legend5=dict(traceorder="normal", x=1.02, y=0.12, bgcolor="rgba(0,0,0,0)")
        )
        
        # ล็อกช่วงกรอบสเกลแกน Y ให้ตรงตามมาตรฐานขอบเขตจริงของเครื่องจักรแต่ละสถานีควบคุม
        ...
        # กล่องที่ 4: แกนซ้าย Oxygen บังคับล็อกช่วงที่ 0 ถึง 200 ppm แน่นอน / แกนขวา N2 Flow เป็นอิสระออโต้สเกลเต็มกำลัง
        ...
        # กล่องที่ 5: Dew Point โหมด Free Scale (Auto-Range) ยืดขยายอิสระตามความชื้นระบบลมจริง
        ...
        st.error("❌ ลоจิกสตรีมมิ่งไม่พบชุดพารามิเตอร์จำนวน 23 ช่องสัญญาณในไฟล์นี้")
else:
    st.info("💡 กรุณาอัปโหลดไฟล์บันทึกสัญญาณ (.DAD) เพื่อแสดงแผงควบคุมเวอร์ชันถอดรหัสสอบเทียบสเกลเสร็จสมบูรณ์")
