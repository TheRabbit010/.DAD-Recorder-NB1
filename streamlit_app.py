...
st.title("🏭 Factory Process Master Dashboard - Production Stable")
st.subheader("พล็อตกราฟกระบวนการผลิต 5 ชั้น ล็อกช่วงสเกลจริงและซิงค์คอลัมน์สมบูรณ์")
...
if uploaded_file is not None:
    ...
    # 1. ลอจิกถอดรหัสขูดข้อมูลตัวเลขความละเอียดสูง (Deep Structural Miner)
    all_numbers = re.findall(r'[-+]?\d*\.\d+(?:[eE][-+]?\d+)?|\b\d{1,4}\b', text_data)
    numeric_stream = [float(n) for n in all_numbers]
    
    # กรองล้างเฉพาะค่าขยะภายนอกพิกัดเครื่องมือวัด (ช่วงอุตสาหกรรม -150 ถึง 3000)
    clean_stream = [n for n in numeric_stream if -120.0 <= n <= 3000.0]
    
    # ล็อกจำนวนช่องสัญญาณ 23 ช่องมาตรฐาน Yokogawa ดึงพารามิเตอร์ครอบคลุม
    ...
    if len(clean_stream) >= detected_channels:
        ...
        st.success(f"🔓 ถอดรหัสไฟล์และจัดเรียงโครงสร้างทั้ง {detected_channels} ช่องสัญญาณ สำเร็จเรียบร้อย!")

        # ฟังก์ชันคำนวณปรับช่วงสเกลตัวเลข (Min-Max Rescaling) ให้ตรงหน้างานจริงโดยไม่มีค่าสูญหาย
        ...
        # 2. เริ่มสร้างโครงสร้าง Subplots แบบ 5 ชั้นแนวตั้ง (แกนเวลารวมลิงก์กันอัตโนมัติ)
        fig = make_subplots(
            rows=5, 
            cols=1, 
            shared_xaxes=True, 
        ...
        # ----------------------------------------------------
        # กล่องที่ 2: Heating Zone 1-7 (Top - ช่วงสเกล 400 - 650 °C)
        # ----------------------------------------------------
        heat_start_idx = 5  # สล็อตจุดเริ่มต้นช่องอุณหภูมิเตาเผา
        for i in range(0, 7):
        ...
        # ----------------------------------------------------
        # กล่องที่ 3: Heating Zone 8-14 (Bottom - ช่วงสเกล 400 - 650 °C)
        # ----------------------------------------------------
        for i in range(7, 14):
            ch_name = f'CH_{heat_start_idx + i}'
            if ch_name in df.columns:
        ...
        # ----------------------------------------------------
        # กล่องที่ 4: Oxygen (แกนซ้าย 0-200) & N2 Flow (แกนขวาออโต้สเกล) [รวมกลุ่ม Legend ไว้ด้านขวารวมกัน]
        # ----------------------------------------------------
        ...
        # 3. ตั้งค่าการจัดวางผังหน้าต่างแผงควบคุม (Layout) แยกสัดส่วน Legend Box ตามแนวระดับสายตา
        fig.update_layout(
            template="plotly_dark",
            height=1100, 
            hovermode="x unified",
            title_text="Yokogawa Process Analyzer Master Dashboard (Stable Core Mode)",
            ...
            legend4=dict(traceorder="normal", x=1.02, y=0.35, bgcolor="rgba(0,0,0,0)"), # รวม Oxygen & N2 Flow อยู่ฝั่งเดียวกันด้านขวา
            legend5=dict(traceorder="normal", x=1.02, y=0.12, bgcolor="rgba(0,0,0,0)")
        )
        
        # ล็อกช่วงขอบเขตความปลอดภัยของแกน Y ประจำสถานีวัดอุตสาหกรรมตรงหน้างานจริง
        fig.update_yaxes(title_text="Dryer Temp (°C)", range=[-10, 420], row=1, col=1)
        fig.update_yaxes(title_text="Heating Top (°C)", range=[380, 680], row=2, col=1)   # ขยายขอบสเกล 400-650 เล็กน้อยให้เส้นไม่ติดขอบ
        fig.update_yaxes(title_text="Heating Bottom (°C)", range=[380, 680], row=3, col=1) # ขยายขอบสเกล 400-650 เล็กน้อยให้เส้นไม่ติดขอบ
    ...
    st.info("💡 กรุณาอัปโหลดไฟล์บันทึกสัญญาณ (.DAD) เพื่อพล็อตกราฟควบคุมกระบวนการผลิตเวอร์ชันเสถียรสูงสุด")
