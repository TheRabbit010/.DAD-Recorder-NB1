...
st.title("🏭 Factory Process Dashboard - Dynamic Scale Restorer")
st.subheader("ปลดล็อกสเกลแกน Y เพื่อคืนค่าความชันและไดนามิกของเส้นกราฟที่แท้จริง")
...
if uploaded_file is not None:
    ...
    # 1. ปรับลอจิกการขูดตัวเลขให้แม่นยำขึ้น โดยจับเฉพาะกลุ่มตัวเลขที่เป็นพารามิเตอร์การวัดจริง
    # ตัดเลขจำนวนเต็มมหาศาลที่เป็น Index หรือที่อยู่ไบนารีของไฟล์ Yokogawa ออกไป
    all_numbers = re.findall(r'[-+]?\d*\.\d+(?:[eE][-+]?\d+)?|\b\d{1,3}\b', text_data)
    numeric_stream = [float(n) for n in all_numbers]
    
    # [ปรับช่วงฟิลเตอร์ให้กระชับ] เน้นเฉพาะสเกลอุณหภูมิ, ออกซิเจน, อัตราไหล และจุดน้ำค้างที่แท้จริง
    ...
    if len(clean_stream) >= detected_channels:
        ...
        # เมนูตั้งค่าเวลาด้านซ้ายมือ (Sidebar)
        ...
        st.success(f"🔓 ถอดรหัสคลื่นสัญญาณสำเร็จ! ตรวจพบข้อมูล {len(df)} แถว (เปิดระบบ Auto-Scaling บนหน้าจอ)")

        # 2. สร้างโครงสร้าง Subplots 4 ชั้นแนวตั้ง
        fig = make_subplots(
            rows=4, 
            cols=1, 
            shared_xaxes=True, 
        ...
        # 3. ตั้งค่า Layout และเปิดโหมด Auto-Scale แยกทุกกล่องย่อยเด็ดขาด (ลบขีดจำกัดสเกลคงที่ออก)
        fig.update_layout(
            template="plotly_dark",
            height=950,
            hovermode="x unified",
            title_text="Yokogawa Process Analyzer Dashboard (Dynamic Scale Mode)"
        )
        
        # บังคับเปิดระบบปรับสเกลอัตโนมัติตามค่าจริงต่ำสุด-สูงสุดของข้อมูลในกล่องนั้นๆ (ไม่ฟิกซ์สเกลที่ 0 หรือ 50)
        ...
        st.error("❌ ข้อมูลพารามิเตอร์กระบวนการผลิตลดลงต่ำกว่าขีดจำกัดโครงสร้าง 23 ช่องสัญญาณ")
else:
    st.info("💡 กรุณาอัปโหลดไฟล์บันทึกสัญญาณ (.DAD) เพื่อพล็อตกราฟกระบวนการผลิต")
