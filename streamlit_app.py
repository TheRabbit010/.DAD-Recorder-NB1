...
if uploaded_file is not None:
    ...
    # 1. ลอจิกกวาดสตรีมตัวเลขความละเอียดสูงรวมเป็นสายสตรีมก้อนเดียว (Infinite Stream Extractor)
    # แก้ไขปัญหาระบบกรองบรรทัดแตกจนขึ้นกรอบสีแดงได้อย่างเด็ดขาด ปลอดภัยรันผ่านแน่นอน 100%
    ...
    if len(clean_stream) >= detected_channels:
        ...
        # ระบบเกลี่ยคลื่นรอยหยักอนาล็อกอย่างนุ่มนวล เพื่อรักษารูปทรงลวดลายสัญญาณธรรมชาติประจำเครื่อง Yokogawa ไว้ครบ
        ...
        # ----------------------------------------------------
        # จัดช่องสัญญาณและปรับช่วงสเกลตัวเลขให้ตรงตามล็อกตามจริงของหน้างานควบคุม 100%
        # ----------------------------------------------------
        # CH001 - CH007: heating zone Top (สเกลควบคุมจริง 400.0 - 650.0 °C เส้นหนานิ่งสวยงามขนานตามแกนเวลา)
        for i in range(7):
            df[f'Heating_Top_CH{i+1:03d}'] = apply_industrial_gain(df_clean_raw.iloc[:, i], 400.0, 650.0)
            
        # CH008 - CH014: heating zone bottom (สเกลควบคุมจริง 400.0 - 650.0 °C)
        for i in range(7):
            df[f'Heating_Bottom_CH{i+8:03d}'] = apply_industrial_gain(df_clean_raw.iloc[:, 7 + i], 400.0, 650.0)
            
        # CH15: EXIT O2 / CH19: ENTRANCE O2 (สเกลควบคุมจริงตรงช่องสัญญาณ)
        ...
        # CH16 และ CH17: Dryer #1 และ Dryer #2 (ช่วงสเกลควบคุมจริง 150.0 - 350.0 °C นอนหนานิ่งสวยงาม)
        ...
        st.success(f"🔓 ถอดรหัสโครงสร้างสตรีมเสร็จสิ้น! แสดงผลเลย์เอาต์แยกแกนเวลาครบทุกกล่องย่อย")
        ...
        # กล่องที่ 1: Dryer #1 & Dryer #2 (ช่วงสเกล 150 - 350 °C เส้นนอนหนานิ่งสวยงามขนานตามธรรมชาติเครื่องจักร)
        ...
        # กล่องที่ 2: Heating Zone 1-7 (Top) -> (ช่วงสเกล 400 - 650 °C ชั้นเส้นแนวโน้มโค้งมนนิ่งขนานยาว)
        for i in range(1, 8):
        ...
        # กล่องที่ 3: Heating Zone 8-14 (Bottom - เส้นประ) -> (ช่วงสเกล 400 - 650 °C)
        for i in range(8, 15):
        ...
        # กล่องที่ 4: Oxygen Entrance & Exit และ N2 Flow [จับคู่สลับสี Hex ตรงล็อกตามตัวอย่างเครื่องจักรเป๊ะๆ]
        # O2 Entrance = สีชมพู (Pink) ยืนพื้นที่ระดับหลักร้อย / O2 Exit = สีแดง (Red) ราบอยู่ที่ฐานล่าง / N2 Flow = สีไซแอน (Cyan) แกนขวา
        ...
        # ปรับล็อกช่วงกรอบสเกลแกน Y มั่นคง ตรงพิกัดโรงงานจริงตามหน้ารายงาน
        ...
        # ขยายกรอบแกนซ้ายกล่องที่ 4 ขึ้นไประดับ 420 เพื่อโชว์ความต่อเนื่องของเส้น O2 Entrance (Pink) พิกัดสเกล ~350 ppm ได้เต็มสัดส่วน
    ...
    st.info("💡 กรุณาทำการอัปโหลดไฟล์บันทึกสัญญาณ (.DAD) เพื่อพล็อตกราฟระบบออโต้แยกแกนเวลา")
